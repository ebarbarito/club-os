-- Modificaciones 8/9/26:
-- - register_dispensa/update_dispensa: si la forma de pago (en plata real,
--   no crédito virtual) supera el total de la dispensa, el excedente
--   generaba un movimiento de ledger de más pero nunca quedaba como saldo
--   a favor del socio. Se agrega la misma lógica de "sobrante -> crédito
--   general" que ya tiene pay_dispensa_batch. El sobrante de un pago hecho
--   con una cuenta virtual (crédito ya existente) sigue sin generar nada
--   nuevo -- mismo criterio que en pay_dispensa_batch.
