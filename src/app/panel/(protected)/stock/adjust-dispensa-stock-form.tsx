...
import { adjustDispensaStock } from './actions';
...
export function AdjustDispensaStockForm({ strainId, currentGrams, unit }: { strainId: string; currentGrams: number; unit: string }) {
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      const res = await adjustDispensaStock(formData);
      if (res?.error) {
        setError(res.error);
        return;
      }
  ...
  return (
    <form action={submit} className="space-y-3">
      ...
      <p className="text-sm text-text-soft">Stock dispensa actual: {currentGrams} {unit}</p>
      <div>
      ...
      <div>
        <label className={labelCls}>Motivo (queda en Asientos)</label>
        <input name="note" required className={inputCls} />
      </div>
      ...
      >
        {pending ? 'Guardando…' : 'Ajustar stock dispensa'}
      </button>
    </form>
  );
}
